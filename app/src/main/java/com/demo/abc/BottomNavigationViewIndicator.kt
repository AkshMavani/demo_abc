...
class BottomNavigationViewIndicator @JvmOverloads constructor(
    ...
    init {
        if (attrs == null) {
            ...
            context.obtainStyledAttributes(attrs, R.styleable.BottomNavigationViewIndicator).apply {
    ...
    override fun onAttachedToWindow() {
        ...
        val parentView = parent as? View ?: return attachedError("Unable to find the parent view")
        val child = parentView.findViewById<View?>(targetId)

        if (child !is ListenableBottomNavigationView) return attachedError("Invalid view $child, the app:targetBottomNavigation must reference a ListenableBottomNavigationView")
