...
import com.gpfreetech.awesomescanne.R;
...
public final class ScannerView extends ViewGroup {
    ...
    private float minZoomLevel = 0.5f;
    ...
    private GpCodeScanner mGpCodeScanner;
    ...
    private void initialize(@NonNull final Context context, @Nullable final AttributeSet attrs,
        ...
        mAutoFocusButton.setImageResource(R.drawable.ic_auto_focus_on);
        ...
        if (attrs == null) {
            ...
            try {
                ...
                setAnimatedViewFinder(a.getBoolean(R.styleable.ScannerView_isAnimated,
                        DEFAULT_IS_ANIMATED));
            } finally {
                if (a != null) {
                    a.recycle();
    ...
    public boolean onTouchEvent(@NonNull final MotionEvent event) {
        final GpCodeScanner gpCodeScanner = mGpCodeScanner;
        final Rect frameRect = getFrameRect();
        final int x = (int) event.getX();
        final int y = (int) event.getY();
        if (gpCodeScanner != null && frameRect != null &&
                gpCodeScanner.isAutoFocusSupportedOrUnknown() && gpCodeScanner.isTouchFocusEnabled() &&
                event.getAction() == MotionEvent.ACTION_DOWN && frameRect.isPointInside(x, y)) {
            final int areaSize = mFocusAreaSize;
            gpCodeScanner.performTouchFocus(
    ...
    /**
     *
     * @param isAnimatedScanner isAnimate
     */
    public void setAnimatedViewFinder(boolean isAnimatedScanner){
        setQrAnimation(isAnimatedScanner);
    }
    ...
    public void setCodeScanner(@NonNull final GpCodeScanner codeScanner) {
        if (mGpCodeScanner != null) {
            throw new IllegalStateException("Code scanner has already been set");
        }
        mGpCodeScanner = codeScanner;
        setAutoFocusEnabled(codeScanner.isAutoFocusEnabled());
        setFlashEnabled(codeScanner.isFlashEnabled());
    }

    public void setAutoFocusEnabled(final boolean enabled) {
        mAutoFocusButton.setImageResource(enabled ? R.drawable.ic_auto_focus_on :
                R.drawable.ic_auto_focus_off);
    }
    ...
    private final class AutoFocusClickListener implements OnClickListener {
        ...
        public void onClick(final View view) {
            final GpCodeScanner scanner = mGpCodeScanner;
    ...
    public void setQrAnimation(boolean isAnimate) {
        if (isAnimate) {
            getViewFinderView().startAnimation();
        } else {
            getViewFinderView().stopAnimation();
        }
    }
    public void setZoomLevel(float zoomLevel) {
        ...
        // Update the camera zoom level here
        // You may need to use the Camera API or a library like ZXing to achieve this
    }
